...
/**
  * @brief  重设 OLED 显示偏移（防止因异常导致的显示偏移）
  * @param  offset 偏移量，通常为 0
  * @retval 无
  */
...
/**
  * @brief  OLED重置显示位置（起始行 + 偏移归零）
  * @note   防止 GPIOB 干扰导致 SSD1306 画面偏移
  * @param  无
  * @retval 无
  */
void OLED_ResetOffset(void)
{
	OLED_WriteCommand(0x40);	//设置显示起始行 = 0
	OLED_WriteCommand(0xD3);	//设置显示偏移
